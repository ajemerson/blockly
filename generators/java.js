...
Blockly.Java.ORDER_MEMBER = 2.1;          // . []
Blockly.Java.ORDER_FUNCTION_CALL = 2.2;   // ()
...
Blockly.Java.ORDER_RELATIONAL = 11;       // in, not in, is, is not,
                                            //     <, <=, >, >=, <>, !=, ==
Blockly.Java.ORDER_LOGICAL_NOT = 12;      // not
Blockly.Java.ORDER_LOGICAL_AND = 13;      // and
Blockly.Java.ORDER_LOGICAL_OR = 14;       // or
Blockly.Java.ORDER_CONDITIONAL = 15;      // if else
Blockly.Java.ORDER_LAMBDA = 16;           // lambda
...
Blockly.Java.init = function(workspace) {
   ...
   */
  Blockly.Java.PASS = this.INDENT + 'pass\n';
...
Blockly.Java.quote_ = function(string) {
  ...
  // Follow the CJava behaviour of repr() for a non-byte string.
  var quote = '\'';
  if (string.indexOf('\'') !== -1) {
    if (string.indexOf('"') === -1) {
      quote = '"';
 ...
 */
Blockly.Java.getAdjustedInt = function(block, atId, opt_delta, opt_negate) {
  var delta = opt_delta || 0;
  if (block.workspace.options.oneBasedIndex) {
    delta--;
  }
  var defaultAtIndex = block.workspace.options.oneBasedIndex ? '1' : '0';
  var atOrder = delta ? Blockly.Java.ORDER_ADDITIVE :
      Blockly.Java.ORDER_NONE;
  var at = Blockly.Java.valueToCode(block, atId, atOrder) || defaultAtIndex;
