...
Blockly.Java['procedures_defreturn'] = function(block) {
  ...
  for (var i = 0, variable; variable = variables[i]; i++) {
    ...
    if (block.arguments_.indexOf(varName) == -1) {
      globals.push(Blockly.Java.variableDB_.getName(varName,
  ...
  for (var i = 0; i < devVarList.length; i++) {
    globals.push(Blockly.Java.variableDB_.getName(devVarList[i],
        Blockly.Names.DEVELOPER_VARIABLE_TYPE));
  }

  globals = globals.length ?
      Blockly.Java.INDENT + 'global ' + globals.join(', ') + '\n' : '';
  ...
  if (Blockly.Java.STATEMENT_PREFIX) {
    ...
    branch = Blockly.Java.prefixLines(
        Blockly.Java.STATEMENT_PREFIX.replace(
            /%1/g, '\'' + id + '\''), Blockly.Java.INDENT) + branch;
  }
